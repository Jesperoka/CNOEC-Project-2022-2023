...
if __name__ == "__main__":
    ...
    # Simulation setup
    phaseLag = pi/9
    T_p_0 = 273.15 + 20*sin(-phaseLag) + 10*uniform(0.5, 1.5) # Kelvin
    t0 = 0
    tf = 1000
    tspan = [t0, tf]
    epsilon = 0.18
    ...
    T_a = [273.15 + 20*sin(2*pi*(t/tf) - phaseLag) + 10*uniform(0.999, 1.001) for t in range(0, tf + 1)]
    ...
    # Basic plot
    plotHelpers.standardizedPlot([[t, T_p], T_a, G], 
        labels=["Panel Temperature (T_p)", "Ambient Temperature (T_a)", "Solar Irradiance (G)"],
        xlabel="time",
        ylabel="temperature (K)",
        ylabel2="irradiance (W m^-2)",
